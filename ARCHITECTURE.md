...
- `point_cloud_ply`: Three.js point cloud view with orbit, pan, zoom, point size, color mode, points/cameras/path/grid/axes controls, and reconstruction metadata when `metadata/reconstruction.json` exists.
...
The job writes `metadata/reconstruction.json` with `mode: reconstruction`, `artifact_type: point_cloud_ply`, `is_reconstruction: true`, input/registered frame counts, sparse point counts, COLMAP workspace metadata, camera intrinsics, registered image poses, camera centers, camera path, trajectory bounds, run params, and quality notes. It fails with setup guidance when `colmap.exe` is missing instead of producing placeholder geometry.

Viewer controls include large-view mode, reset/fit, front/side/top/default camera presets, screenshot capture, point size, color mode, points/cameras/path/grid/axes toggles, artifact stats, and debug warnings.

Artifact ordering intentionally puts real reconstruction artifacts before splats/GLB, debug frame planes, debug reports, and placeholder exports so the UI does not visually promote debug data as the primary result.
